...
6. If the raw image file is available, upload it first:
   `POST http://34.124.244.233/zenbali/api/agent/uploads/event-image`
7. Read `data.image_url` from the upload response and map it to `e267` / `image_url`.
8. Send the full JSON payload to:
   `POST http://34.124.244.233/zenbali/api/agent/events`
...
## Upload Example
```bash
curl -X POST 'http://34.124.244.233/zenbali/api/agent/uploads/event-image' \
...
## Event Post Example
```bash
curl -X POST 'http://34.124.244.233/zenbali/api/agent/events' \
